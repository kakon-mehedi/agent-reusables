...
Tool-agnostic definition. Turns a raw BRD section into a structured, service-scoped requirement spec. Entry point of the agentic engineering pipeline (see `workflows/new-service.workflow.yaml`). Use when a service/module needs its requirement-spec.md produced or refreshed from the project's BRD.

## Purpose

Turn the raw BRD into a structured, service-scoped requirement spec for one target service/module.

## Input

- The target service name (and its constituent BRD services, if it's a merge per an existing ADR in the project's `docs/adr/`).
...
3. **Non-Functional Requirements** — pulled from the BRD's global NFR table plus anything service-specific.
4. **Domain Invariants** — business rules that must hold regardless of implementation. These will later seed the DDD Agent's work.
