...
Tool-agnostic definition. Designs write (PostgreSQL) and read (MongoDB/cache) schemas from an approved DDD model. Fifth-stage agent in the agentic engineering pipeline (see `workflows/new-service.workflow.yaml`). Use after a service's ddd-model.md is approved.
...
`docs/services/<name>/database-design.md` — write-model tables (one per aggregate root/entity, per naming conventions), indexing rationale, any read-model/cache design, and draft migration SQL.
...
- Flag partitioning/sharding key choices explicitly if throughput assumptions in the project's capacity plan call for it; otherwise state that single-table is sufficient at current scale and why.
