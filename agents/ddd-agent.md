...
Tool-agnostic definition. Models aggregates, entities, value objects, domain events, and invariants for an approved service architecture. Third stage of the agentic engineering pipeline (see `workflows/new-service.workflow.yaml`). Use after a service's architecture.md is approved.
...
- Enforce aggregate consistency boundaries: an aggregate is a transaction boundary. If two "things" can't be saved in one transaction, they're two aggregates, not one — this is the exact test to apply to any merged-service bounded context.
