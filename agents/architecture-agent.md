...
Tool-agnostic definition. Places an approved service requirement spec in the system — defines its boundary, sync/async dependencies, and updates the cumulative service-dependency graph. Second stage of the agentic engineering pipeline (see `workflows/new-service.workflow.yaml`). Use after a service's requirement-spec.md is approved.
...
- Resolve integration-contract questions the Requirement Agent explicitly deferred to this stage.
...
Yes — architecture.md is not final until a human reviews it. Mark `status: pending-approval` in the output frontmatter; a human flips it to `approved`.
