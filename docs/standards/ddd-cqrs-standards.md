...
- Every new or reused term is checked against the project's ubiquitous-language glossary before being introduced. New terms are proposed additions, not silent introductions.
- Domain invariants (e.g., "a quantity must never go negative") live in the aggregate that owns them, enforced in domain code — never only validated at the API edge.
