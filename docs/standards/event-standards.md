...
- Topic exchange (`<domain>.events`); routing key convention `service.entity.action` (e.g., `order.created`).
...
- Retry budget scales with criticality: high-criticality events get the highest retry count + human paging on final failure; low-criticality events (catalog/search-style) tolerate looser retry.

## Kafka (adopted per consumer group, where justified)

- Only for consumers that need replay or partitioned high-throughput consumption (e.g., analytics, recommendation).
- Partition key = aggregate id, to preserve per-entity ordering.
- Migration is strangler-pattern: dual-publish from the Outbox during transition, cut over one consumer group at a time. Transactional flows that need strict ordering/DLQ semantics can stay on RabbitMQ permanently — this is case-by-case, not wholesale.
