...
- `Idempotency-Key` header is **mandatory** on every money-moving `POST` (payments, refunds, and other non-retry-safe writes).

## gRPC

- Reserved for internal, high-throughput synchronous calls only (e.g., an inventory reserve check). Never exposed publicly.
